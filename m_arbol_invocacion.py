...
def mostrar_arbol_invocaciones(datos_csv, funcion, string = ""):
    '''[Autor: Andrés Kübler]
    ...
    #Si la funcion recorrida tiene invocaciones las recorre
        ...
        for invocacion_de_funcion in range(len(invocaciones)):
            ...
            #Si la invocacion recorrida es la primera de las invocaciones, la función se llama a si misma y pasa el string como variable
                ...
                mostrar_arbol_invocaciones(datos_csv, invocaciones[invocacion_de_funcion], str_invocacion)
            #Sino es la primera, se llama a si misma y pasa la longitud del string pero en espacios
            else:
                mostrar_arbol_invocaciones(datos_csv, invocaciones[invocacion_de_funcion], " " * len(str_invocacion))

def graficar_arbol_invocaciones(datos_csv):
    '''[Autor: Andrés Kübler]'''

    funcion_principal = encontrar_main_archivo(datos_csv)
    if funcion_principal:
        mostrar_arbol_invocaciones(datos_csv, funcion_principal)
    else:
        print("No se encontro la funcion principal.")